...
public class Edge {
    ...
    public Line[] createLine(Vertex v1, Vertex v2) {
        //Line line = new Line();
        //Polygon.
        this.line.setStartX(v1.circle.getX() + sizeof2);
        this.line.setStartY(v1.circle.getY() + sizeof2);
        this.line.setEndX(v2.circle.getX() + sizeof2);
        this.line.setEndY(v2.circle.getY() + sizeof2);
        this.line.setStyle("-fx-stroke-width:2");

        //Media sound = new Media(new File("").toURI().toString());
        //MediaPlayer mediaPlayer = new MediaPlayer(sound);
        //mediaPlayer.play();
        double arrowHeight=sizeof1, arrowWidth=sizeof1;
        double dx, dy,dx1,dy1;
        int m;
        if (v2.circle.getX() > v1.circle.getX()) {
            dx = v2.circle.getX() - v1.circle.getX();
            dy = v2.circle.getY() - v1.circle.getY();
            dx1=v1.circle.getX()+dx/2+sizeof1+dx/4;
            m = 1;
        } else {
            dx = -v2.circle.getX() + v1.circle.getX();
            dy = -v2.circle.getY() + v1.circle.getY();
            dx1=v2.circle.getX()+dx/2+10-dx/4;
            m = -1;
        }

        if(v2.circle.getY() > v1.circle.getY()){
            dy1= v1.circle.getY()+(v2.circle.getY() - v1.circle.getY())/2+10+(v2.circle.getY() - v1.circle.getY())/4;
        }
        else {
            dy1 = v2.circle.getY()+(-v2.circle.getY() + v1.circle.getY())/2+10+(v2.circle.getY() - v1.circle.getY())/4;
        }
        ...
        Point p5=new Point((int)(v2.circle.getX()-m*(sizeof1)*Math.cos(angle))+sizeof2,(int)(v2.circle.getY()-m*(sizeof1)*Math.sin(angle))+sizeof2);
        Point p2=new Point((int)(v2.circle.getX()-m*(arrowHeight+sizeof1)*Math.cos(angle)),(int)(v2.circle.getY()-m*(arrowHeight+sizeof1)*Math.sin(angle)));
        angle = Math.atan(((-1) * dx) / dy);
        Point p3=new Point((int)(p2.getX()-(arrowWidth / 2) * Math.cos(angle))+sizeof2, (int)(p2.getY()- (arrowWidth / 2) * Math.sin(angle))+sizeof2);
        Point p4=new Point((int)(p2.getX()+(arrowWidth / 2) * Math.cos(angle))+sizeof2, (int)(p2.getY()+ (arrowWidth / 2) * Math.sin(angle))+sizeof2);
        ...
        Line line2 = new Line();
        line2.setStartX(p4.getX());
        line2.setStartY(p4.getY());
        line2.setEndX(p5.getX());
        line2.setEndY(p5.getY());
        line2.setStyle("-fx-stroke-width:2");
    ...
    public void changeLine (Vertex v1, Vertex v2, boolean weightChange) {
        ...
        this.line.setEndY(v2.circle.getY() + sizeof2);
        if (!weightChange) {
            this.weight = (double)sqrt(((v1.circle.getX() - v2.circle.getX()) * (v1.circle.getX() - v2.circle.getX()))
        ...
        //Line line3 = new Line();
        allLine[2].setStartX(p4.getX());
        allLine[2].setStartY(p4.getY());
        allLine[2].setEndX(p5.getX());
        allLine[2].setEndY(p5.getY());
